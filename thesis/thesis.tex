...
can group them. The algorithm from \cite{fomin_golovach_panolan_2020} has defined the clustering metric
...
\begin{itemize}
    \item \url{https://www.ibm.com/support/pages/clustering-binary-data-k-means-should-be-avoided}
    \item \url{https://drops.dagstuhl.de/opus/volltexte/2018/9623/pdf/LIPIcs-MFCS-2018-41.pdf}
    \item \url{https://bora.uib.no/bora-xmlui/bitstream/handle/11250/2735169/archive.pdf?sequence=1}
    \item The PDF: Zohar-master
\end{itemize}
...
While clustering, we can assume that the centres will be of a distance greater than $k$. Therefore we can
start by grouping the vectors into sets, where any vector in a set with more than one element, is of
a distance less than or equal to $k$, to at least one other vector. This will then result in equal vectors
being grouped together, and thus if there are many duplicate vectors some of them can be discarded. Thus all
the vectors have to be constructed into clusters of equal vectors denoted by a new set $I^* = \{I_1, \dots, I_t\}$, henceforth
called the \textit{initial clusters}; subsequently limiting the size of each cluster \textit{s.t.} $|I_i| \leq k+2$.
Also note that if $t > k+r$ we already have no solution so return NO. Finally all vectors of distance $d_H \leq k$ 
can be assumed to be in the same cluster, we should move all vectors of initial clusters with distance 
$d_H \leq k$ into a new "final" cluster. While performing this task, if at any point,
the algorithm produces more than $r$ "final" clusters, then return NO.

As we have now created the first clusters we're mostly done, but there is still one reduction that can be made.
From the definition of the centres being of the smallest distance to all it's vectors, it must follow that
any \textit{uniform row} will result in the same result. Meaning if $\forall k$ in $\Sigma_{i=1}^{m} v_i \in A_k = 0 \vee m$,
the row can be discarded from further processing.

\input{algorithms/kernel.tex}

When reducing the initial matrix, the algorithm has now produced a set of clusters $L$, where any vector
...
but as this allows for multi processing, and a more separate design, this will not be done. Instead each cluster will be 
treated as a separate matrix and processed separately in the branching algorithm multiple times with different 
parameters $k$ and $r$ and a solution to be made dynamically later in \mref{sec:dynamic-result}
...
with $S = S \cup \{s\}$. \textit{Note: the original paper \cite{fomin_golovach_panolan_2020} specified that the
branching should be on permutations using "Agrees with", which was here skipped for practical reasons.}
...
Given \mref{alg:recon} we should either get a NO, or a final solution. This final solution will then be only a set of centres,
