...
To my old favourite teachers, you're the reason I could even start my degree. You did so much for so many, and
you might not even remember, but you saw me for who I was, not just another student. Jørn Hafver, you taught
...
\subsection{Overview}
The thesis is organized as follows. In \mref{sec:prelim} the most basic notations and definitions which will be used in
...
in \mref{sec:conclusion} we have the conclusion, and a suggestion for further work. The entire work and results is
...
The goal of the pre processing algorithm is to reduce the input matrix by following certain reduction steps
...
matrix $A$ has such a solution with the same $r$ and $k$. Hence acting like a Turing kernel as in \mref{def:turing-kernelization}.
...
From \mref{lem:5} we know that a combination of the solutions of the
...
thus we actually have a \mref{def:turing-kernelization}.
...
We have in \mref{sec:algo:branching} given the algorithm for finding the min \#clusters that can be obtained for $A_i$,
