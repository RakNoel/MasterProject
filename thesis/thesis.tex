...
We will apply the following reduction-rules

\begin{reductionrule}
    Any duplicate vector must be in the same cluster. Therefore it follows that more than $k+1$ equal vector
...
When we reduce the matrix we will produce instance of clusters, where any vector in a cluster is at max k distance to all
other vectors in the same cluster $\forall d_H(v, u) \in L_i \leq k$. Each of these clusters  can now be treated as it's 
own sub-matrices $A_1, \dots, A_i$ of the original matrix $A$.
