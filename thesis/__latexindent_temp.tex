...
we can run the algorithm multiple times over $A_i$ with increasing parameter $k$,
...
\[ f(k) =
  \begin{cases}
    \text{NO - Instance}   & \quad \text{if min\{r\} with } k \geq r+1 \\
    \text{YES - Instance}  & \quad \text{if min\{r\} with } k \leq r
  \end{cases}
\]
...
$f_i(k^\prime)$ =
\begin{cases}{lcl}
    min \#clusters that can be obtained for $A_i$ using at most $k^\prime$ \\
    $r+1$, if $\#\text{clusters} > r$ 
\end{cases}
\]

With the functions $f_i(k^\prime)$ we can define the table as such 
\\
\begin{tabular}{lcl}
    $T[i, k^\prime]$ & $\rightarrow$ & min \#clusters that can be obtained for $A_1, \dots, A_i$, using at most $k^\prime$ \\
